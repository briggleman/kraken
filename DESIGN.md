...
**The Dim-What-Is-Lit Rule.** The shared veil is `rgba(2,9,14,0.78)`, and it is a real dimmer over the pane — a panel at `#0a1e2c` lands near `#040e15` beneath it, roughly halved. Over the **trench** ground a drill-in and every sheet use (`#030d16` → `#02090E`), it does almost nothing: `#030d16` resolves to about `#020a10`, a change of one or two levels per channel. Near-black cannot dim near-black. A modal opened from inside a drill-in therefore reads as floating on an undimmed backdrop, and no amount of alpha on a colour this close to the ground will fix it — the veil would have to change hue or the card would have to carry its own separation. **This is a known limitation, not a documented behaviour:** verify a modal against the surface it will actually open over, with that surface open, before calling the separation good.
...
- **Don't** judge an overlay's veil against the pane. `rgba(2,9,14,0.78)` halves a panel and does essentially nothing to the trench ground a drill-in and every sheet use — open the surface it will really sit over before deciding the separation reads.
