...
The reason is a real misread: an idle fleet reported itself honestly (one jiffy per sampling window
on 32 cores, a NIC averaging bytes per second, nothing placed yet) and the all-zero board read as a
*dead* one. Truth should not look like failure, and the house already had the sentence for it — the
One Light Rule's "a thing at rest loses its light", spent on a number. It is not a new state:
nothing is prevented by an idle node, so nothing here takes Caution Violet, and the glow is simply
reserved for numbers that are alive.

Three boundaries make it honest. **Unknown stays the em-dash** — "the host cannot report this" is a
different sentence from "this is zero", and it keeps its blanked chart. **A stale zero must not
dim as resting**, because "the number stopped arriving" is not "the number is zero". And it applies
to **rates only** — cpu and network. Memory and disk are levels, and an empty level is a fact about
capacity rather than a thing at rest. The test the app applies is what the operator *sees*: cpu
dims when the reading rounds to `0`, network when it formats to `0.0`, not when the underlying float
...
- **Do** make a multi-step flow's outcome the app's actual state: the node the wizard brings online is the node the fleet then shows, and the specs setup auto-imports are the ones the spec sheet lists. A setup that ends somewhere the rest of the product never mentions is describing a different install.
