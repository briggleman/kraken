...
| `KRAKEN_IMAGE_PULL` | `--image-pull` | `always` | When to pull a game-server image: `always` (try the registry on install and on an operator-driven start, falling back to the local copy if it fails), `if-not-present` (only fetch what the node lacks), or `never` (never contact a registry). Crash auto-restarts never pull. |
