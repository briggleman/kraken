...
resolves — which behind a proxy with no `KRAKEN_TRUSTED_PROXIES` set is *the
proxy*, for everybody. That is the single configuration mistake that turns both
limiters into one shared bucket one stranger can exhaust for the whole fleet.
...
- **Every JSON request body is capped at 4 MiB.** Sized off the largest
  ...
  Without a cap, any authenticated caller could pin arbitrary Panel memory with
  one request, because `json.Decoder` reads until the body ends.
- **Uploads are capped at 64 MiB** plus a megabyte of multipart framing, and
  ...
  this one authenticated request could fill the Panel's disk. Use SFTP for
  anything larger.
...
unauthenticated caller — a rejected download token is the example. That is how
an operator turns them on when actually diagnosing something, rather than
carrying an attacker-controlled write into the log level everybody reads.
...
The Agent's log is separate: `journalctl -u kraken-agent` on Linux,
...
alongside `/healthz` and `/readyz` — so it belongs on a network your scrapers
can reach and strangers cannot, exactly like the rest of the Panel.
...
The last two are the reason the quiet `Debug` lines are also counted: they give
an operator the signal without turning the log level down. A 429'd login never
