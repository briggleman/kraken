<script lang="ts">
  ...
  const logLines = $derived(showingInstall ? installLines : stream.lines);
      ...
      <section class="console" aria-label="Server station">
        ...
        <div class="stn-panel p-console">
          <div class="console-log" id="consoleLog" bind:this={consoleLog}>
            {#if showingInstall}
              <div><span class="t">—</span>[panel] install log from {installWhen} — held in panel memory only, so a panel restart loses it</div>
