<script lang="ts">
  ...
  // its count and the line's title carries the roll call instead.
  const namesInline = $derived(
    !!containers && containers.items.length > 0 && containers.items.length <= DRIFT_INLINE_MAX,
  );
  const driftNames = $derived(namesInline && containers ? containers.items : []);
  ...
  // Removals this node owes: servers retired or deleted while it could not be
  ...
  const driftTitle = $derived(
    containers && containers.items.length > DRIFT_INLINE_MAX
      ? `${containers.word}: ` + containers.items.map(containerTitle).join(", ")
      : undefined,
  );
